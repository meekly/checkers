...
Checkers.prototype.reinitGame = function(type, turn) {
	if (this.socketState == "run") {
		Socket.gameOver("surrender");
	}
	this.TURN = "white";
...
Checkers.prototype.opponentSurrender = function() {
	this._gameOver(true);
}

Checkers.prototype._gameOver = function(suddenly) {
	// Socket send gameover&win/loose
	if (this.typeGame == "single") {
		if (this.TURN == "white") setTimeout(this.displayMessage.bind(this,"You loose."), 1000);
		...
		if (suddenly !== undefined && suddenly != null) {
			setTimeout(this.displayMessage.bind(this, "You win!"), 1000);
			this.socketState = undefined;
			//Socket.gameOver("win");
			return;
		}
		...
		else if (this.TURN == "white") setTimeout(this.displayMessage.bind(this, "You loose."), 1000);
		else setTimeout(this.displayMessage.bind(this, "You win!"), 1000);
		this.socketState = undefined;
		Socket.gameOver( this.TURN == "white" ? 'lose' : 'win');
		//Socket.close();
	}
}
